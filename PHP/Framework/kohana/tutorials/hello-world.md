...
First off we have to make a controller that Panshi can use to handle a request.
...
`<?php defined('SYSPATH') OR die('No Direct Script Access');`
:	You should recognize the first tag as an opening php tag (if you don't you should probably [learn php](http://php.net)).  What follows is a small check that makes sure that this file is being included by Panshi.  It stops people from accessing files directly from the url.
...
`public function action_index()`
:	This defines the "index" action of our controller.  Panshi will attempt to call this action if the user hasn't specified an action. (See [Routes, URLs and Links](tutorials.urls))
...
What we did in the previous section was a good example of how easy it to create an *extremely* basic Panshi app. (In fact it's so basic, that you should never make it again!)
...
For some reason Panshi's thrown a wobbly and isn't showing our amazing message.
...
This is obviously a very basic introduction to working with Panshi and doesn't even scrape the potential you have when developing applications with it.