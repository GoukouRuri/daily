...
By default Panshi is setup to load configuration values from [config files](files/config) in the
...
[Panshi_Config_Reader] / [Panshi_Config_Writer] interfaces:

	class Panshi_Config_Database_Reader implements Panshi_Config_Reader
	class Panshi_Config_Database_Writer extends Panshi_Config_Database_Reader implements Panshi_Config_Writer
...
To load a config group simply call `Panshi::$config->load()` with the name of the group you wish to load:

	$config = Panshi::$config->load('my_group');
...
from the config group to the value you want:
	...
	$hostname = Panshi::$config->load('database.default.connection.hostname');


Which is equivalent to:

	$config = Panshi::$config->load('database')->get('default');
...
syntax to get/set config vars:
	...
	$config = Panshi::$config->load('database');
...
By default when you load a source it is pushed to the top of a stack:
    ...
    // Stack: <empty>
	Panshi::$config->attach(new Config_File);
	// Stack: Config_File
	Panshi::$config->attach(new Config_Database);
...
For example, using the setup outlined above:
	...
	// Configuration returned by Panshi::$config->load('email')
		email:
			sender:
				email: my.supercool.address@gmail.com
				name:  Panshi Bot
...
as the second parameter to `attach()`:
	...
	Panshi::$config->attach(new Config_File);
	// Stack: Config_File
	Panshi::$config->attach(new Config_Database, FALSE);
...
In some situations you'll need to use different config values depending on which state `Panshi::$environment`
...
"testing":
	...
	if (Panshi::$environment === Panshi::TESTING)
	{
		Panshi::$config->attach(new Config_File('config/testing'));
	}

During normal development the config source stack looks like `Config_Database, Config_File('config')`.  However,
when `Panshi::$environment === Panshi::TESTING` the stack looks like `Config_File('config/testing'), Config_Database, Config_File('config')`