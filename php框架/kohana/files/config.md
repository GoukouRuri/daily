...
If the above configuration file was called `myconf.php`, you could access it using:
    ...
    $config = Panshi::$config->load('myconf');
...
We could now call `Panshi::$config->load('site.title')` to get the site name, and `Panshi::$config->load('site.analytics')` to get the analytics code.
...
You could then do the following:
	...
	// In your controller
	$view->versions = Panshi::$config->load('versions');
