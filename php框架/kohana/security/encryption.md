...
Panshi supports built-in encryption and decryption via the [Encrypt] class, which is a convenient wrapper for the [Mcrypt library](http://www.php.net/mcrypt).
...
The default Encryption config file that ships with Panshi 3.2.x looks like this:
...
[!!] Raw encrypted strings usually won't print in a browser, and may not store properly in a VARCHAR or TEXT field. For this reason, Panshi's Encrypt class automatically calls base64_encode on encode, and base64_decode on decode, to prevent this problem.
...
even without changing your key, cipher and mode.  This is because Panshi adds some random entropy before encoding it with your value.
